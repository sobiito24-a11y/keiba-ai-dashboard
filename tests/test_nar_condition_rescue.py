...
from core.prediction_snapshot import build_event_snapshot,keiba_bytes,load_keiba,restore_prediction_result,race_snapshot_from_result
...
def test_urawa_saved_real_prediction_rescue_web_png_and_invariance(race_id,expected):
    ...
    assert f==before
    # Existing schema round-trip, using production serializer APIs.
    snap=build_event_snapshot([race_snapshot_from_result(result)])
    restored=restore_prediction_result(load_keiba(keiba_bytes(snap))['races'][0])
    assert app.nar_comparison_from_result(restored)['condition_rescue']==rescue
