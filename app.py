...
def main() -> None:
    ...
    # Dashboard orchestration only. Prediction and selected-race rendering use
    # the canonical Mobile functions imported in this module.
    from core.dashboard_application import render_dashboard

    render_dashboard(render_colab_style_result)
...
def render_full_field_comparison(
    ...
    *,
    ...
    if is_jra:
        ...
        st.caption("NARは純能力順位1〜5位を正式Top5として表示します。購入判定は順位・印を変更しません。")
    st.subheader("全頭横比較")
...
def full_field_comparison_html(comparison: dict[str, Any], *, include_body_weight: bool = False) -> str:
    rows = comparison.get("rows") or []
    if not rows:
        return '<div class="ka-dashboard-card">比較できる出走馬データがありません。</div>'
    race_mode = clean_text(comparison.get("race_mode")).lower()
...
def market_horse_card_html(row: dict[str, Any], race_mode: str) -> str:
    ...
    ability_rank = (
        ...
        if is_jra
        else clean_text(pick(row, "nar_pure_ability_rank", *VER3_ABILITY_RANK_COLUMNS)) or "—"
    )
    current_rank = (
