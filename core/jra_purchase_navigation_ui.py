...
def jra_purchase_navigation_html(navigation: Mapping[str, Any]) -> str:
    ...
    if status in {"対象外", "判定材料不足"}:
        ...
        roles = {"強軸": ("中心", "本線", "狙い"), "上位混戦": ("本線", "押さえ", "狙い"), "評価分裂": ("本線参考", "狙い")}[status]
        ...
        parts.append('<div class="ka-note">◎は現行Top5の最上位評価です。レース構造判定とは別です。軸ありはTop3＋✔︎、複数候補はTop5＋✔︎。見送り寄りは参考表示です。穴注意は自動追加せず、候補を広げる場合の参考です。休養注意は順位・スコア・印を変更しません。</div></details>')
